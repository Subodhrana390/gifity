  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        dispatch(setError(errorData.error || "Failed to analyze repository. Please check the URL and try again."));
      }
    } catch (err) {
  ...
  const handleGenerateReadme = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        dispatch(setError(errorData.error || "Failed to generate README. Please try again."));
      }
    } catch (err) {
      dispatch(setError("Network error. Please check your connection and try again."));
    } finally {
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-8 text-center">
            ...
            <p className="text-gray-300">
              Paste a GitHub repository URL to analyze and generate professional documentation
            </p>
          </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1 relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <svg className="h-5 w-5 text-gray-400" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
                  </svg>
                </div>
                <input
          ...
          {analysis && (
            <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700 mb-8 shadow-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 className="text-lg font-semibold text-blue-400 mb-2">{analysis.name}</h3>
                  <p className="text-gray-300 mb-4">{analysis.description || "No description available"}</p>
                  
                  <div className="space-y-3">
                    <div className="flex items-center">
                      <span className="text-gray-400 w-24">Language:</span>
                      <span className="ml-2 text-white font-medium">{analysis.language || "Not specified"}</span>
                    </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="ml-2 text-white">
                        {analysis.updatedAt ? new Date(analysis.updatedAt).toLocaleDateString() : "N/A"}
                      </span>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-900 rounded-md p-3 max-h-40 overflow-y-auto">
                    {analysis.files && analysis.files.slice(0, 10).map((file, index) => (
                      <div key={index} className="text-sm text-gray-300 py-1 flex items-center">
